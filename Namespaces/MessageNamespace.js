...
class MessageNamespace {
    ...
    async reply(message, content) {
        try {
            ...
            await axios.post(
                `${this.client.apiBase}/channels/${message.channel_id}/messages`,
                payload,
                {
                    headers: { Authorization: `Bot ${this.client.token}` },
                },
    ...
    async delete(message) {
        try {
            await axios.delete(
                `${this.client.apiBase}/channels/${message.channel_id}/messages/${message.id}`,
                {
                    headers: { Authorization: `Bot ${this.client.token}` },
                },
            );
            ...
            console.error(
                `Error deleting message ${message.id} from channel ${message.channel_id}:`,
                error,
            );
        }
    }
    ...
    async react(message, emoji) {
        try {
            ...
            console.error(
                `Error reacting to message ${message.id} in channel ${message.channel_id} with ${emoji}:`,
                error,
            );
        }
    }
